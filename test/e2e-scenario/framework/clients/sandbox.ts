...
export class SandboxClient {
  ...
  exec(
    ...
    validateSandboxName(name);
    // `openshell sandbox exec` takes the sandbox name via `--name` (positional
    // args after `--` are the in-sandbox command). Mirrors the legacy bash
    // test's invocation: `openshell sandbox exec --name $SANDBOX -- $@`.
