...
Resolve a gateway failure through the component that owns the gateway lifecycle.
Start with `$$nemoclaw status` or `$$nemoclaw status --json`.
For a NemoClaw-managed Linux packaged service, inspect the managed user service that exists on the host.
NemoClaw can use the upstream package unit or the NemoClaw-staged unit, depending on how the gateway service was installed:
...
For `externally-supervised`, inspect the exact `supervisor.serviceName` reported by the selected authority.
...
For `systemd-system`, use the system manager:

```bash
systemctl status <serviceName>
```

Bring the gateway up, leave exactly one verified process holding the configured port, and rerun `$$nemoclaw onboard`.
