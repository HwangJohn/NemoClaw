...
When Ollama reports a loaded-model context length, NemoClaw uses it for the `model.context_length` written to `/sandbox/.hermes/config.yaml` unless you set `NEMOCLAW_CONTEXT_WINDOW`.
Hermes requires at least `64000` tokens, so NemoClaw raises lower auto-detected Ollama values to `64000` before the sandbox is built.
An explicit `NEMOCLAW_CONTEXT_WINDOW` below `64000` stops config generation instead of producing a sandbox that fails on first inference.
</AgentOnly>
...
<AgentOnly variant="hermes">
When Ollama reports a context length below `64000` and `NEMOCLAW_CONTEXT_WINDOW` is unset, NemoClaw writes `model.context_length: 64000` so Hermes meets its minimum context requirement.
</AgentOnly>
