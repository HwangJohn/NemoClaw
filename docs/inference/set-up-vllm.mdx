...
If it serves a large unquantized checkpoint or an unusually long context window, it can leave too little unified-memory headroom for the host and surface as `NVRM NV_ERR_NO_MEMORY`, SSH loss, or a hard host freeze under agent/tool load.
