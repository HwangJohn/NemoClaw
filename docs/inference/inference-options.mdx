...
For advanced vLLM options that are not in the NemoClaw registry yet, export `NEMOCLAW_VLLM_EXTRA_ARGS_JSON` as a JSON array of individual `vllm serve` tokens.
NemoClaw validates the array before pulling images or downloading models, shell-quotes each token, and appends the tokens after the registry defaults.
