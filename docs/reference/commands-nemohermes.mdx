...
For guidance on when to use `nemohermes` versus the underlying `openshell` CLI, see [CLI Selection Guide](cli-selection-guide).
...
Caller stdin is forwarded by default for compatibility with terminal, pipe, and SSH workflows.
For scripted SSH or CI one-shot commands that should not read caller stdin, pass `--no-stdin` so the sandbox command cannot wait on an inherited pipe.
Pass `--stdin` to force forwarding when a wrapper or alias sets `--no-stdin`.

```bash
printf 'hello\n' | nemohermes my-assistant exec -- cat
...
| `--stdin` / `--no-stdin` | Force caller stdin forwarding or closure. Default: forward stdin |
...
Caller stdin is forwarded by default for compatibility with terminal, pipe, and SSH workflows.
For scripted SSH or CI one-shot commands that should not read caller stdin, pass `--no-stdin` so the command cannot wait on an inherited pipe.
Pass `--stdin` to force forwarding when a wrapper or alias sets `--no-stdin`.
