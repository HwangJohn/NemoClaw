...
| `NEMOCLAW_VLLM_EXTRA_ARGS_JSON` | JSON array of strings | Appends advanced operator-owned tokens to the managed `vllm serve` command after NemoClaw's registry defaults. Example: `["--max-num-seqs","2"]`. Malformed JSON or non-string tokens fail before Docker work starts. |
