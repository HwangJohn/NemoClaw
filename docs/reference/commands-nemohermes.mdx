...
Export Hermes session JSONL from a running sandbox to the host.

For Hermes sandboxes, the command calls the native `hermes sessions export -` command inside the sandbox and writes each returned JSONL record to a browsable host directory.
By default the destination is `./sessions-<sandbox>/`.
Use `--agent hermes` when you want to be explicit; omitting `--agent` is equivalent for a registered Hermes sandbox.

```bash
nemohermes my-assistant sessions export
nemohermes my-assistant sessions export --agent hermes
nemohermes my-assistant sessions export --out ./sessions-hermes --json
```

| Flag | Description |
|------|-------------|
| `--agent hermes` | Optional explicit Hermes agent selection. Other agent ids are refused for Hermes exports. |
| `--out <path>` | Host directory destination. Defaults to `./sessions-<sandbox>/`. |
| `--json` | Print the export manifest as JSON instead of a status line. |

Hermes exports currently do not support positional key filters, `--format tar`, or `--include-trajectory`.
Session JSONL can contain pasted secrets (API keys, tokens), so exported files are written owner-only (`0600`).
