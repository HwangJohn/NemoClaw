...
During onboarding, NemoClaw raises loaded-model context lengths below `64000` to `64000` when `NEMOCLAW_CONTEXT_WINDOW` is unset.
Explicit smaller values are rejected during config generation.
Force a larger context length:
