...
### Docker Desktop WSL sandbox bridge cannot reach the gateway

On Docker Desktop WSL, sandbox containers reach host services through Docker Desktop's `host-gateway` route, not through the WSL distro's `127.0.0.1` loopback.
When NemoClaw detects this runtime, it binds the OpenShell Docker-driver gateway to `0.0.0.0` while keeping the CLI-facing gateway endpoint on `127.0.0.1`.
This lets sandbox containers connect to `host.openshell.internal:<gateway-port>` during onboarding.
If your environment requires a loopback-only gateway, set `NEMOCLAW_GATEWAY_BIND_ADDRESS=127.0.0.1`; Docker Desktop WSL sandbox callbacks may then fail until the gateway is reachable through another route.
