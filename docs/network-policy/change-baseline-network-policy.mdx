...
Each entry in the `network_policies` section defines a named endpoint group.
Choose one policy name for the map key and repeat it in the required `name` field:
...
Use these fields when you author or modify an entry:

`name`
: Required policy entry name.
Use the same value as the `network_policies` key.

`endpoints`
: Host and port entries that the sandbox can reach.
Each endpoint owns its protocol, enforcement mode, method and path rules, and encoded-slash handling.

`rules`
: Endpoint-level HTTP methods and paths that the endpoint permits.
Put `rules` inside each endpoint item, not next to `endpoints` or `binaries`.

`allow_encoded_slash`
: Endpoint-level option that allows percent-encoded slashes such as `%2F` in request paths.
...
`binaries`
: Executables allowed to use the endpoint, written as objects with a `path` value.
...
- [Network Policies](../../reference/network-policies) summarizes the baseline policy entries and tiers.
